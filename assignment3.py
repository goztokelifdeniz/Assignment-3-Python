...
def mark_far_neighbors(list5, row_num, col_num):
    ...
    else:  # so that even when row_num is very small, list can be checked properly
        checking_times = 3
    ...
    if not game_is_over:
        ...
        if 1 <= input_row <= row_num and 1 <= input_column <= col_num:
            if cell_has_a_valid_neighbor(list10, col_num, input_row, input_column):
                ...
                if len(row_erased_list) == 0:  # if there is no number left on board after erasing the empty rows
                    print_score(score)
                    print("")
                    print("Game over")
                else:
                    col_erased_list, col_num = erase_empty_columns_and_update_col_num(row_erased_list, row_num, col_num)
                    if len(col_erased_list) == 0:  # if there is no number left on board after erasing the empty columns
                        print_score(score)
                        print("")
                        print("Game over")
                    else:
...
def main():
    row_amount = find_row_amount()
    column_amount = find_column_amount()
    board_list = turn_file_into_list()
    score = 0

    # printing the board and the score as head start
    print_board(board_list, column_amount)
    print_score(score)
    print("")
    add_invalid_marking_as_a_start(board_list)

    play_board_game(board_list, row_amount, column_amount, score)
